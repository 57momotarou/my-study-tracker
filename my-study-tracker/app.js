...
// コマnの「出席認定期限」を返す
// 専門=木曜12:00、教養/外国語=火曜12:00、+2週間後が締切
...
// 学期開始日から最初の締切曜日までのオフセット
const startDow = start.getDay();
let daysToFirst = (deadlineDow - startDow + 7) % 7;
// 開始日当日が締切曜日の場合はその週とする
const firstDeadline = new Date(start);
firstDeadline.setDate(start.getDate() + daysToFirst);
firstDeadline.setHours(12, 0, 0, 0);

// コマnは第n週の締切日 → さらに+2週間が出席認定期限
const lessonDeadline = new Date(firstDeadline);
lessonDeadline.setDate(firstDeadline.getDate() + (lessonNum - 1) * 7 + 14);
return lessonDeadline;
}

// 今日時点で「期限が来ているコマ数」= 今日やるべき累計コマ数
...
// 今日やるべきコマ数（繰り越し優先）
// = 期限切れ未完了コマ + 今週末までに期限が来るコマ
function getTodayRecommended(subject, semester, done) {
const now = new Date();
// 今週の締切日（次の締切曜日12:00）を求める
const deadlineDow = subject.deadline_type === ‘専門’ ? 4 : 2;
const todayDow = now.getDay();
let daysToDeadline = (deadlineDow - todayDow + 7) % 7;
...
if (dl <= thisWeekDeadline) count++;
else break;
}
// 繰り越し分（期限切れ未完了）を含む、今週末締切までの累計
...
// アラート（出席認定期限切れコマを遅刻として警告）
const alertsEl = document.getElementById(‘today-alerts’);
alertsEl.innerHTML = ‘’;
subjects.forEach(s => {
const target = getTodayTarget(s, sem); // 期限が来ている累計コマ数
const done = getCompletedLessons(s.code);
const late = target - done; // 期限切れ未完了コマ数
if (late >= 3) {
...
const done = getCompletedLessons(s.code);
const target = getTodayTarget(s, sem);       // 期限切れ累計
const recommended = getTodayRecommended(s, sem, done); // 今週末までの推奨
const pct = Math.round(done / s.lessons * 100);
const late = target - done;           // 遅刻コマ数
const needThisWeek = recommended - done; // 今週やるべき残コマ
let badgeClass = ‘badge-ok’, badgeText = ‘✅ OK’;
if (late >= 1) {
badgeClass = ‘badge-danger’;
badgeText = `🔴 遅刻${late}コマ`;
} else if (needThisWeek >= 1) {
badgeClass = ‘badge-warn’;
badgeText = `🟡 今週あと${needThisWeek}コマ`;
}
return `<div class="today-subject-item"> <div class="today-subject-dot" style="background:${getCategoryColor(s.category)}"></div> <div class="today-subject-info"> <div class="today-subject-name">${s.name}</div> <div class="today-subject-meta">${s.credits}単位 ・ ${done}/${s.lessons}コマ完了${late > 0 ?` ・ <span style="color:var(--red)">遅刻${late}コマ</span>` : ''}</div> <div class="prog-wrap"><div class="prog-bar" style="width:${pct}%;background:${getCategoryColor(s.category)}"></div></div> </div> <span class="today-subject-badge ${badgeClass}">${badgeText}</span> </div>`;
}).join(’’);
...
weekEl.innerHTML += ` <div class="week-cell ${isToday ? 'is-today' : ''} ${isRest ? 'is-rest' : ''}"> <div class="week-day-label">${DOW_LABELS[d]}</div> ${dotHtml || '<div style="height:6px"></div>'} </div>`;
}
...
progEl.innerHTML = ` <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:6px;"> <span style="font-size:13px;color:var(--text2)">${done} / ${total} コマ完了</span> <span style="font-family:'Space Mono',monospace;font-size:20px;font-weight:700;color:var(--amber)">${pct}%</span> </div> <div class="prog-wrap" style="height:10px"> <div class="prog-bar" style="width:${pct}%;background:var(--amber)"></div> </div>`;
}
}
...
listEl.innerHTML += `<div class="subject-row ${isChecked ? 'checked' : ''}" data-code="${s.code}" style="${isChecked ?`–check-color:${color}`: ''}"> <div class="subject-row-check" style="${isChecked ?`background:${color};border-color:${color}` : ''}"> <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#000" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg> </div> <div class="subject-row-info"> <div class="subject-row-name">${s.name}</div> <div class="subject-row-meta">${s.code} ・ ${s.type} ・${s.lessons}回</div> </div> <div class="subject-row-credits">${s.credits}単位</div> </div>`;
});
});
...
listEl.innerHTML = subjects.map(s => `<div class="enrolled-item"> <div class="enrolled-dot" style="background:${getCategoryColor(s.category)}"></div> <div class="enrolled-name">${s.name}</div> <div class="enrolled-credits">${s.credits}単位</div> </div>`).join(’’);
...
statsEl.innerHTML = ` <div class="stat-box"> <div class="stat-box-num" style="color:var(--amber)">${totalCredits}</div> <div class="stat-box-label">合計単位</div> </div> <div class="stat-box"> <div class="stat-box-num">${senmonCount}</div> <div class="stat-box-label">専門科目</div> </div> <div class="stat-box"> <div class="stat-box-num">${kyoyoCount + foreignCount}</div> <div class="stat-box-label">教養+外国語</div> </div>`;
}
...
summaryEl.innerHTML = ` <div class="badge-summary-grid"> <div class="badge-summary-item"> <div class="badge-summary-num" style="color:#cd7f32">${bronze}</div> <div class="badge-summary-label">🥉 ブロンズ</div> </div> <div class="badge-summary-item"> <div class="badge-summary-num" style="color:#94a3b8">${silver}</div> <div class="badge-summary-label">🥈 シルバー</div> </div> <div class="badge-summary-item"> <div class="badge-summary-num" style="color:var(--amber)">${gold}</div> <div class="badge-summary-label">🥇 ゴールド</div> </div> <div class="badge-summary-item"> <div class="badge-summary-num" style="color:#67e8f9">${platinum}</div> <div class="badge-summary-label">💎 プラチナ</div> </div> </div> <div style="font-size:12px;color:var(--text3);text-align:center;margin-top:6px;"> 合計 ${earned.length} / ${BADGES.length} バッジ取得済み </div>`;
...
html += `<div class="badge-card ${isEarned ? 'earned' : ''}" style="${isEarned ?`color:${levelCfg.color}`: 'opacity:0.6'}"> <div class="badge-icon">${isEarned ? levelCfg.icon : '🔒'}</div> <div class="badge-info"> <div class="badge-level-tag" style="${isEarned ?`background:${levelCfg.bg};color:${levelCfg.color}`: 'background:var(--bg3);color:var(--text3)'}">${levelCfg.label}</div> <div class="badge-name">${badge.name}</div> ${badge.requirements?.codes ?`
...
`:`<div class="badge-meta">${badge.requirements?.description || ‘’}</div>`} </div> </div>`;
});
...
subjects.forEach(s => {
const done = getCompletedLessons(s.code);
const target = getTodayTarget(s, sem);           // 期限切れ累計コマ数
const recommended = getTodayRecommended(s, sem, done); // 今週末推奨累計
const pct = Math.round(done / s.lessons * 100);
const late = Math.max(0, target - done);         // 遅刻コマ数
const color = getCategoryColor(s.category);
...
// コマボタン（色分け：完了=カテゴリ色、遅刻未完=赤、今週期限=アンバー、未来=グレー）
...
for (let i = 1; i <= s.lessons; i++) {
  ...
  if (isDone) {
    btnStyle = `background:${color};color:#000`;
    btnClass += ' done';
  } else if (isLate) {
    btnStyle = `background:var(--red-dim);color:var(--red);border:1px solid var(--red)`;
  } else if (isThisWeek) {
    btnStyle = `background:var(--amber-dim);color:var(--amber);border:1px solid var(--amber)`;
  }
  btnHtml += `
    <button class="${btnClass}"
      onclick="toggleLesson('${s.code}', ${i}, ${semId})"
      style="${btnStyle}"
      title="${isLate ? '遅刻中' : isThisWeek ? '今週の出席認定対象' : ''}"
    >${i}</button>`;
}
btnHtml += '</div>';
...
listEl.innerHTML += `
  <div class="progress-subject-card">
    ...
    <div class="ps-meta">${done} / ${s.lessons} コマ完了${late > 0 ? ` ・ <span style="color:var(--red)">遅刻${late}コマ</span>` : ''}</div>
    <div class="prog-wrap">
