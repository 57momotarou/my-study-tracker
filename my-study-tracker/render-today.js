...
// ============================================================
// ============================================================
// TODAY 時間割カード（時間割スケジュール基準）
// ・今日の曜日に割り当てられた科目を「今日やること」として表示
// ・遅刻中の科目は「積み残し」として上部に表示
// ・今日の予定が全完了なら完了メッセージ
// ============================================================
function renderTodayTimetable(subjects, sem, semId) {
  ...
  const withState = subjects.map(s => {
    ...
    const ttDay   = getTimetableDay(s.code, semId); // 0=月〜5=土
    // 今日の時間割か（月〜土のみ、0=日は除く）
    const isScheduledToday = ttDay !== undefined && ttDay === (todayDow - 1) && todayDow >= 1 && todayDow <= 6;
    // 今日の受講が済んでいるか（時間割科目で次のコマが今日の推奨範囲内なら）
    const nextLesson = doneLes + 1;
    return { s, doneCh, doneLes, target, rec, late, ttDay, isScheduledToday, nextLesson };
  });
  ...
  if (overdueItems.length > 0) {
    ttEl.innerHTML += `<div style="font-size:11px;font-weight:700;color:var(--red);margin-bottom:8px;padding:6px 10px;background:var(--red-dim);border-radius:6px;border-left:3px solid var(--red)">⚠️ 積み残し — まず終わらせよう</div>`;
    overdueItems
      .sort((a, b) => b.late - a.late)
      .forEach(item => _renderTodayCard(ttEl, item, sem, semId));
  }

  // ── 今日の時間割科目 ──
  if (todayItems.length > 0) {
    if (overdueItems.length > 0) {
      ttEl.innerHTML += `<div style="font-size:11px;color:var(--text3);margin:12px 0 8px">✅ 積み残し後は次の予定へ</div>`;
    } else {
...
// 科目カード1枚を描画（今日タブ用）
function _renderTodayCard(ttEl, item, sem, semId) {
