// ============================================================
// my-study-tracker - render-badges.js（画像通りのビジュアルツリー）
// ============================================================
...
function renderBadgesPage() {
  ...
  ['教養','外国語'].forEach(function(cat) {
    ...
    catBadges.forEach(function(badge) {
      var ie=isEarned(badge), lcfg=LCFG[badge.level]||LCFG.bronze, p=getProg(badge);
...
function buildBadgeTree(getBadge, isEarned, getProg, LCFG) {
  ...
  function nd(id, w) {
    ...
    var bc=ie?lcfg.color:'var(--border)', bg=ie?lcfg.color+'22':'var(--bg3)', tc=ie?lcfg.color:'var(--text3)';
    var fi=w>=80?'16px':w>=68?'14px':w>=56?'13px':'11px';
    var fn=w>=80?'9px':w>=68?'9px':w>=56?'8px':'7px';
    var bar='';
    if (p.total>0) {
      bar='<div style="font-size:7px;color:var(--text3);margin-top:2px">'+p.done+'/'+p.total+'</div>'
        +'<div style="width:80%;height:2px;background:var(--bg2);border-radius:99px;overflow:hidden;margin-top:1px">'
        +'<div style="width:'+pct+'%;height:100%;background:'+(ie?lcfg.color:'var(--text3)')+'"></div></div>';
    }
    return '<div style="display:flex;flex-direction:column;align-items:center;padding:6px 3px;border-radius:10px;'
      +'border:2px solid '+bc+';background:'+bg+';width:'+w+'px;flex-shrink:0;box-sizing:border-box;cursor:default">'
      +'<div style="font-size:'+fi+'">'+icon+'</div>'
      +'<div style="font-size:'+fn+';font-weight:700;color:'+tc+';text-align:center;line-height:1.3;margin-top:2px;width:100%;overflow:hidden">'+b.name+'</div>'
      +bar+'</div>';
  }
  ...
  // 横分岐線（子が複数の時）
  function hbranch(count, nodeW, gap) {
    var total = nodeW*count + gap*(count-1);
    var half  = total/2;
    return '<div style="position:relative;height:14px;width:'+total+'px;margin:0 auto">'
      +'<div style="position:absolute;top:0;left:50%;transform:translateX(-50%);width:2px;height:14px;background:var(--border)"></div>'
      +'<div style="position:absolute;top:8px;left:'+(nodeW/2)+'px;right:'+(nodeW/2)+'px;height:2px;background:var(--border)"></div>'
      +'</div>';
  }
  ...
  // ── ルート：IT総合学基礎 ──
  html += '<div style="display:flex;justify-content:center">'+nd('badge-it-bronze',90)+'</div>';
  html += vl;
  ...
  // ────────── テクノロジー系（左） ──────────
  html += '<div style="flex:1;min-width:0;background:linear-gradient(135deg,rgba(59,130,246,0.08),rgba(96,165,250,0.04));'
    +'border:1px solid rgba(96,165,250,0.25);border-radius:12px;padding:10px 8px">';
  ...
  // テクノロジー基礎Ⅰ ＋ 数学基礎（横並び・同列）
  html += '<div style="display:flex;justify-content:center;gap:6px">';
  html += nd('badge-tech1-bronze',64);
  html += nd('badge-math-bronze',64);
  html += '</div>';
  // Ⅰの下のみ縦線
  html += '<div style="display:flex;justify-content:flex-start;padding-left:calc(50% - 32px - 3px)">'+vls+'</div>';
  // テクノロジー基礎Ⅱ
  ...
  html += nd('badge-network-gold',56)+nd('badge-security-gold',56)+nd('badge-software-gold',56)+nd('badge-ai-gold',56);
  html += '</div>';
  html += vl;
  // プラチナ4種
  html += '<div style="display:flex;justify-content:center;gap:4px">';
  html += nd('badge-network-platinum',56)+nd('badge-security-platinum',56)+nd('badge-software-platinum',56)+nd('badge-ai-platinum',56);
  html += '</div>';
  // IT総合学プラチナ（点線区切りで別行）
  html += '<div style="border-top:1px dashed rgba(96,165,250,0.3);margin-top:8px;padding-top:8px;display:flex;justify-content:center">';
  html += nd('badge-it-platinum',80);
  html += '</div>';

  html += '</div>'; // end テクノロジー系

  // ────────── ビジネス系（右） ──────────
  html += '<div style="flex:1;min-width:0;background:linear-gradient(135deg,rgba(239,68,68,0.08),rgba(248,113,113,0.04));'
    +'border:1px solid rgba(248,113,113,0.25);border-radius:12px;padding:10px 8px">';
  ...
  html += '<div style="display:flex;justify-content:center">'+nd('badge-biz-silver',80)+'</div>'+vl;
  // ゴールド5種
  html += '<div style="display:flex;justify-content:center;gap:3px">';
  html += nd('badge-genai-gold',50)+nd('badge-dm-gold',50)+nd('badge-mgmt-gold',50)+nd('badge-startup-gold',50)+nd('badge-biz2-gold',50);
  html += '</div>'+vl;
  // プラチナ4種（生成AIはなし・左スペース）
  html += '<div style="display:flex;justify-content:center;gap:3px">';
  html += '<div style="width:50px;flex-shrink:0"></div>'; // 生成AIの分のスペース
  html += nd('badge-dm-platinum',50)+nd('badge-mgmt-platinum',50)+nd('badge-startup-platinum',50)+nd('badge-biz2-platinum',50);
  html += '</div>';
