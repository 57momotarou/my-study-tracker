...
function renderProgressPage() {
  ...
  SEMESTERS.forEach(sem => {
    if (!getEnrolledCodes(sem.id).length) return;
    ...
    btn.addEventListener('click', () => { state.currentSemesterId=sem.id; saveState(); renderProgressPage(); });
...
function buildProgressCard(s, sem, semId) {
  ...
  // アイコン（SVGは別途DOM要素で作成→参照取得できるように）
  ...
  // アイコンをpct右横に追加
  const pctWrap = headerDiv.querySelector('div > div:first-child > div:last-child');
  if (pctWrap) pctWrap.appendChild(iconSvg);
  ...
  // ★ attachAccordionで接続
  attachAccordion(headerDiv, gridContainer, () => iconSvg);
